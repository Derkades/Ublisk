...
public class TestNPC extends NPC {
	...
	public void talk(final UPlayer player) {
		...
		new NPCMenu("Test Menu", 3*9, qp){
			...
			@Override
			public void open() {
				this.items.put(10, new ItemBuilder(Material.STONE).name("Some random option").lore("Some lore!", "Lines 2").create());
				this.items.put(11, new ItemBuilder(Material.OAK_PLANKS).name("Option without lore").create());
				super.open();
			}
